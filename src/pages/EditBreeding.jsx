...
import { Save, Calendar, Info, Activity, FileText, User, Tag, IndianRupee, ChevronDown } from 'lucide-react';
...
const EditBreeding = () => {
  ...
  useEffect(() => {
    const fetchAnimalsAndRecord = async () => {
      try {
        ...
        const { data: recordData } = await supabase.from('breeding_records')
          .select('*')
          .eq('id', id)
          .single();
        ...
        if (recordData) {
          ...
          try {
            if (recordData.note) {
              const parsed = JSON.parse(recordData.note);
              symptoms = parsed.symptoms || '';
              doctor_name = parsed.doctor_name || '';
              semen_breed = parsed.semen_breed || '';
              cost = parsed.cost || '';
              note = parsed.real_note || parsed.note || '';
            }
          } catch (e) {
            note = recordData.note || '';
          }

          const doctorExists = (doctorsData || []).some(doc => doc.name === doctor_name);
          const manual = doctor_name !== '' && !doctorExists;
          setIsManualDoctor(manual);

          const presetBreeds = ['Gir', 'Sahiwal', 'Jersey', 'HF', 'Kankrej', 'Murrah', 'Mehsana', 'Jafarabadi'];
          const breedExists = presetBreeds.includes(semen_breed);
          const manualBreed = semen_breed !== '' && !breedExists;
          setIsManualBreed(manualBreed);

          setFormData({
            animal_id: recordData.animal_id || '',
            breeding_date: recordData.breeding_date || '',
            type: recordData.type || 'Natural',
            bull_details: recordData.bull_details || '',
            symptoms,
            doctor_name,
            semen_breed,
            cost,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { error } = await supabase.from('breeding_records').update(updateData).eq('id', id);
      if (error) throw error;

      // Schedule reminders when breeding record is successfully updated
      try {
        const animal = animals.find(a => a.id === formData.animal_id);
        const animalTag = animal ? (animal.tag_number || animal.name) : 'Cattle';
        await scheduleBreedingReminders(animalTag, formData.breeding_date, id);
      } catch (err) {
        console.error('Failed to update breeding notification:', err);
      }
      
      alert(t('breeding_record_updated_successfully', 'Breeding record updated successfully!'));
      navigate('/breeding');
    } catch (err) {
  ...
  return (
    <div className="animate-fade-in" style={{ background: '#F8FAFC', minHeight: '100vh', paddingTop: 'calc(var(--safe-top) + 88px)', paddingBottom: '40px' }}>
      <PageHeader title={t('edit_heat_register')} showBack={true} />
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {['Pending', 'Success', 'Failed', 'Delivered', 'Pregnant'].map(st => (
                  <button 
                    key={st} 
                    type="button" 
                    ...
                    style={{ 
                      ...
                      fontWeight: '800', 
                      cursor: 'pointer', 
                      background: formData.status === st ? '#0B1F4D' : '#F1F5F9', 
                      color: formData.status === st ? 'white' : '#64748B',
